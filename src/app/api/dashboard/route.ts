  ...
  try {
    ...
    return NextResponse.json({
      metrics: {
        ...
        floors: floorsCount,
        rooms: roomsCount,
        occupiedRooms: buildings.reduce((sum: number, b: any) => sum + (b.floors?.reduce((fSum: number, f: any) => fSum + (f.rooms?.filter((r: any) => r.status === 'OCCUPIED').length || 0), 0) || 0), 0),
        vacantRooms: buildings.reduce((sum: number, b: any) => sum + (b.floors?.reduce((fSum: number, f: any) => fSum + (f.rooms?.filter((r: any) => r.status === 'AVAILABLE').length || 0), 0) || 0), 0),
        beds: bedsCount,
